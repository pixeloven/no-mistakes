...
At run admission, no-mistakes captures the explicit repository commit identity (`user.name` and `user.email`) and `commit.gpgsign` policy from the worktree that initiated the push or rerun. Per-worktree values take precedence over repository-local values. The snapshot is stored with the run and applied to pipeline-owned correction commits, rebases, and agent subprocesses for the run's lifetime; an absent value keeps normal Git precedence instead of manufacturing an override. This preserves an explicit `commit.gpgsign=false` as honestly unsigned, an explicit `true` as signed, and the initiating worktree's exact explicit identity without changing user or global Git configuration.
Concurrent autonomous runs require `git config --worktree` support and isolated gate settings. Run setup fails with an upgrade-and-reinitialize diagnostic when Git cannot provide that isolation or an older gate still carries commit identity or signing values in shared config; refreshing the gate with `no-mistakes init` removes those legacy shared values.
