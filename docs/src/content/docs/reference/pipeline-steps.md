...
At admission, a run freezes the registered checkout's effective `commit.gpgsign` boolean without changing Git configuration. That preserves the user's existing signer by default. An operator who explicitly wants an autonomous run to create unsigned commits passes `axi run --unsigned` (or `rerun --unsigned`); this freezes `false` for that run only. Rebase reconstruction, pipeline correction and CI repair commits, and agent subprocesses receive the frozen value through command-local `git -c commit.gpgsign=<value>` or the equivalent environment overlay. No admission or recovery path writes, removes, or requires a persistent signing key. Reruns inherit the selected run's frozen policy unless `--unsigned` is explicitly supplied. Historical runs created by the earlier worktree-scoped protocol retain its fail-closed recovery validation, including refusal of stale or conflicting `commit.gpgsign` configuration; use a fresh run rather than editing their recorded policy.
