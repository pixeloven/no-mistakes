...
At admission, a run requires worktree-scoped Git configuration, captures the registered checkout's explicit worktree-local `commit.gpgsign` setting as a canonical tri-state (`true`, `false`, or unset), and persists it for the run. A rerun requires and inherits the selected run's persisted setting instead of recapturing the checkout, and daemon recovery requires that persisted state and validates that the run worktree still matches it rather than changing it. Explicit `true` and `false` values are propagated command-locally through pipeline correction and CI repair commits, rebases, and agent subprocesses. For unset, admission requires both the worktree and shared repository-local key to be absent, resolves ordinary Git precedence once to a normalized effective boolean, and persists and propagates that boolean command-locally; later config changes therefore cannot alter the run. Recovery rejects a newly stale exact `commit.gpgsign` key without inheriting or clearing it. Enable isolated worktree configuration in the registered checkout with `git config extensions.worktreeConfig true`; setting `git config --worktree commit.gpgsign false` there then keeps autonomous gate commits unsigned without changing global Git configuration.
The hook suppression is limited to those correction-commit invocations. It does not persist a hook override in the registered checkout, run worktree, global Git configuration, or daemon environment; CI repair commits and all other commit paths keep normal hook behavior. Pipeline gates remain authoritative; whether a CI repair returns through the local gates before publication is controlled by [`ci.revalidate_repairs`](/no-mistakes/reference/repo-config/#cirevalidate_repairs).
