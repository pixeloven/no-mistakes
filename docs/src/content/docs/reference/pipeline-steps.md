...
At admission, a run requires worktree-scoped Git configuration, captures the registered checkout's explicit worktree-local `commit.gpgsign` setting as a canonical tri-state (`true`, `false`, or unset), and persists it for the run. A rerun inherits the selected run's persisted setting when present instead of recapturing the checkout, and daemon recovery validates that the run worktree still matches the persisted setting rather than changing it. Explicit `true` and `false` values are propagated command-locally through pipeline correction and CI repair commits, rebases, and agent subprocesses. For unset, admission requires both the worktree and shared repository-local key to be absent, resolves ordinary Git precedence once to a normalized effective boolean, and persists and propagates that boolean command-locally; later config changes therefore cannot alter the run. Recovery rejects a newly stale exact `commit.gpgsign` key without inheriting or clearing it. Setting `git config --worktree commit.gpgsign false` in the registered checkout therefore keeps autonomous gate commits unsigned without changing global Git configuration.
