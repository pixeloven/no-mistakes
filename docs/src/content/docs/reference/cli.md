...
A run that goes terminal (cancelled, failed, or completed without a push stage) after moving the pipeline head leaves the branch `pipeline_owned`. Status offers `next_action.code: recover_custody` only when recovery can establish the same eligibility it will enforce. The recorded head must be a full object ID in the repository's object format and name that exact commit. An equal or ahead local head proves the source locally and can create the local anchor when the gate is unavailable, but any existing gate recovery ref must still match the recorded head. Importing a head that is absent locally requires the repository-bound local gate, a clean worktree, and either the exact non-symbolic run-specific recovery ref or unambiguous evidence for the exact recorded commit in that gate, plus local ancestry or the content-preservation proof described below. A foreign ref that reaches the commit, conflicting reachability, a wrong object type, or a mismatched recovery ref cannot authorize custody. The eligible state reports `safety: blocked_pipeline_owned_recoverable`, the run's terminal `pipeline.status`, and the exact `submitted_head`/`current_head`/`relation` ownership facts.
...
Without positive evidence that the submitted head stayed unchanged, custody is not guessed away. A genuinely absent recorded commit is reported as a missing preserved head; malformed records, conflicting or ambiguous ownership evidence, and import cases with a dirty worktree or genuinely divergent history are reported as invalid or unsafe. Each requires manual reconciliation instead of advertising a recovery that will refuse.
...
`--recover` verifies the run is terminal, anchors the preserved head under `refs/no-mistakes/recover/<run>` in the invoking repository, safely reconciles an available internal gate branch to the recovered local head, and only then stamps custody returned so a fresh run can start.
...
For behind or diverged worktrees, recovery verifies the preserved head at the run-specific recovery ref in the local gate and fetches it into the anchor before moving or refusing. A legacy recorded head without that anchor can be anchored only when the exact commit remains in the gate with unambiguous ownership evidence: it is unreachable from other refs, or the recorded branch is the sole containing ref and points exactly at that commit.
...
This covers a pipeline rebase onto a newer base without requiring the gate branch to have advanced before recovery.
Terminalization pins a verified unpublished pipeline head under a run-specific recovery ref. Before custody is stamped, a present gate branch must already match the recovered local head or be proven by the same content-exact merge check to be wholly represented there. Recovery preserves a replaced gate head under `refs/no-mistakes/recover-gate/<run>` and compare-and-swaps the gate branch to the recovered head; a missing branch is created only if it stays missing. Conflicting, unsafe, symbolic, non-commit, or concurrently changed gate state refuses. If the gate directory is unavailable, the established equal/ahead local-only recovery remains possible; imported recovery still requires the gate. If the recorded head is genuinely missing, status reports manual reconciliation instead of advertising `recover_custody`.
