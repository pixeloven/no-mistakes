...
no-mistakes axi run --intent "the user's goal" --unsigned
```
...
| `--unsigned`    | `bool`   | `false` | Disable commit signing for this run only without changing Git configuration |
...
By default, admission freezes and preserves the registered checkout's effective commit-signing choice. `--unsigned` is the only opt-out: it freezes `commit.gpgsign=false` for this run and applies it command-locally to commit-producing Git subprocesses. It never writes, clears, or requires persistent Git configuration. Supplying it while a signed/default active run already exists is refused rather than silently reattaching with different semantics.
...
It contains `run_id`, `disposition` (`created` or `reused`), `launch_nonce`, `validation_generation`, `branch`, `head_sha`, `submitted_head_sha`, `intent_digest`, and `unsigned`.
...
A conflicting submitted head, validation generation, intent, or unsigned choice is refused.
...
Without positive evidence that the submitted head stayed unchanged, custody is not guessed away. Conflicting evidence and import cases with a dirty worktree retain manual-reconciliation guidance instead of being labeled as a missing preserved head. A clean, genuinely divergent branch has one narrow mechanical path when the exact non-symbolic run-owned gate anchor still pins the verified terminal head, the local head is an exact commit on the same branch, the heads share a merge base, neither contains the other, every gate ref involved is valid, and no newer run owns the branch: status advertises only `no-mistakes axi sync --recover --keep-local`. That transaction leaves the worktree on the local head and keeps the pipeline head reachable under its run-owned recovery ref. Missing, mismatched, symbolic, invalid, dirty, or superseded evidence continues to refuse. When a verified recorded head is absent from both the invoking worktree and an accessible local gate, with compatible recovery refs, status reports `safety: blocked_recover_preserved_head_missing` with the same command; in that separate case the flag is the operator's explicit choice to keep the current local head and discard the missing preserved commits. An unavailable gate remains blocked because absence cannot be established there. The archive-backed keep-local exception has its own stricter proof below.
...
That adoption anchors the pre-recovery local head under `refs/no-mistakes/recover-local/<run>`, then moves the branch with Git operations that refuse on their own rather than after a preceding check: an atomic compare-and-swap on the branch ref, and a working-tree update that aborts instead of overwriting a modified or untracked file. Before custody is stamped, every ordinary recovery also reconciles the gate branch to the selected local result in the same transaction: independently displaced gate work is anchored, the selected commit is staged without invoking receive hooks, the gate update is compare-and-swap, temporary staging is cleaned, and the exact postcondition is verified. A conflicting gate or a race refuses without stamping custody or losing any head.
...
For an older already-stamped `custody_returned` run, inspection verifies the current gate branch before offering a fresh run. A valid gate at the exact local head may proceed. A different valid gate head reports `blocked_custody_returned_gate_stale` and offers `git switch -c <unused-successor-branch> <full-local-head>`; a symbolic, non-commit, or unreadable gate reports `blocked_custody_returned_gate_invalid` and offers manual inspection. Neither refusal moves a ref. Use an actually unused successor branch and never force the stale gate.
...
| `--unsigned` | `bool` | `false` | Disable commit signing for this rerun only without changing Git configuration |
