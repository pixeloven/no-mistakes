...
# A synchronize event races the push that caused it with the later PR-body
# update. Re-read the forge's current PR snapshot for a short, bounded window so
# the check judges the state that will actually be visible to reviewers. This
# is deliberately finite: an absent token, an unavailable API, or a body that
# never settles still fails closed rather than sleeping indefinitely.
...
def event_document() -> dict:
    """Read the complete workflow event document, when the runner supplied it."""
...
def synchronize_event() -> bool:
    return (os.environ.get("GITHUB_EVENT_NAME") or "").strip() == "pull_request" and event_action() == "synchronize"


def synchronize_pr_number(facts: Facts) -> str:
    if facts.number:
        return facts.number
    # GitHub's replay of an old pull_request run can have an empty event
    # payload. The ref remains refs/pull/<number>/merge, which is observation
    # metadata rather than contributor-controlled PR content.
    match = re.fullmatch(r"refs/pull/(\d+)/(?:merge|head)", os.environ.get("GITHUB_REF", "").strip())
...
def current_attestation_head(body: str) -> str:
    ...
    try:
        ...
        return ""
    return payload.get("head_sha", "") if isinstance(payload, dict) and isinstance(payload.get("head_sha"), str) else ""


def read_current_pr(number: str) -> Facts | None:
    """Read the current PR snapshot through GitHub's own API."""
...
def settled_facts(facts: Facts) -> Facts:
    ...
    if not synchronize_event():
        return facts
    number = synchronize_pr_number(facts)
    if not number:
        fail("::error::pull_request synchronize event has no PR number; refusing to judge empty event data.\n")
    expected_head = facts.head_sha
    latest = None
    for attempt in range(SYNCHRONIZE_SETTLE_ATTEMPTS):
        latest = read_current_pr(number)
        if latest is not None and latest.number == "":
            latest.number = number
        if latest is not None and latest.body and latest.head_sha:
            # Never certify an API snapshot for a different event head. A
            # replayed run has no head in its event payload and binds to the
            # current PR head returned by GitHub.
            if expected_head and latest.head_sha != expected_head:
                pass
            # Do not certify the transient pre-edit body. A matching
            # attestation is the publication boundary; all other forms remain
            # eligible to fail after the bounded window expires.
            elif current_attestation_head(latest.body) == latest.head_sha:
                return latest
        if attempt + 1 < SYNCHRONIZE_SETTLE_ATTEMPTS:
            time.sleep(SYNCHRONIZE_SETTLE_DELAY_SECONDS)
    fail("::error::could not read the current pull request snapshot for synchronize; refusing to judge stale or empty event data.\n")
