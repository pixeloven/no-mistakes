...
type Run struct {
	...
	// CommitSigningPolicy and CommitSigningEffective freeze the command-local
	// boolean selected at admission. Historical PR-11 rows may use an empty
	// policy plus an effective value, or an explicit policy with nil effective.
	...
	// LaunchUnsigned records whether --unsigned was explicitly selected. It is
	// distinct from an ordinary effective false and is part of nonce identity.
	LaunchUnsigned bool
...
func (d *DB) ClaimLaunchReceipt(repoID, branch, launchNonce, submittedHeadSHA, validationGeneration, intentDigest, prBaseBranch string, unsignedValues ...bool) (*Run, bool, error) {
	prBaseBranch = strings.TrimSpace(prBaseBranch)
		...
		err := scanRun(d.sql.QueryRow(
			`UPDATE runs SET launch_receipt_claimed_at = ?
			 WHERE repo_id = ? AND branch = ? AND launch_nonce = ?
			   ...
			   AND COALESCE(launch_unsigned, 0) = ?
			   AND (? = '' OR pr_base_branch = ?)
			   AND launch_receipt_claimed_at IS NULL
			 RETURNING `+runColumns,
			now(), repoID, branch, launchNonce, submittedHeadSHA, validationGeneration, intentDigest, unsigned, prBaseBranch, prBaseBranch,
		), r)
		if err == nil {
			return r, true, nil
		}
...
// SetRunCommitSigningPolicy freezes the command-local signing choice for a run.
...
func (d *DB) SetRunLaunchUnsigned(id string, unsigned bool) error {
	_, err := d.sql.Exec(`UPDATE runs SET launch_unsigned = ?, updated_at = ? WHERE id = ?`, unsigned, now(), id)
	if err != nil {
		return fmt.Errorf("set run launch unsigned: %w", err)
	}
	return nil
}
