...
func TestClaimLaunchReceiptBindsExplicitUnsignedChoice(t *testing.T) {
	d := openTestDB(t)
	repo, err := d.InsertRepo("/home/user/unsigned-project", "git@github.com:user/unsigned-project.git", "main")
	if err != nil {
		t.Fatal(err)
	}
	intent := RunIntent{Summary: "intent", Source: RunIntentSourceAgent, Score: 1}
	run, err := d.InsertRunWithIntentAndLaunchNonce(repo.ID, "feature", "head", "base", &intent, "nonce-unsigned", "generation", "digest", "")
	if err != nil {
		t.Fatal(err)
	}
	if err := d.SetRunLaunchUnsigned(run.ID, true); err != nil {
		t.Fatal(err)
	}
	conflict, claimed, err := d.ClaimLaunchReceipt(repo.ID, "feature", "nonce-unsigned", "head", "generation", "digest", "", false)
	if err != nil || claimed || conflict == nil || conflict.ID != run.ID {
		t.Fatalf("mismatch = %#v, %v, %v", conflict, claimed, err)
	}
	if stored, _ := d.GetRun(run.ID); stored.LaunchReceiptClaimedAt != nil {
		t.Fatal("mismatch consumed receipt")
	}
	matched, claimed, err := d.ClaimLaunchReceipt(repo.ID, "feature", "nonce-unsigned", "head", "generation", "digest", "", true)
	if err != nil || !claimed || matched == nil || !matched.LaunchUnsigned {
		t.Fatalf("match = %#v, %v, %v", matched, claimed, err)
	}
}
