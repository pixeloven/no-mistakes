...
CREATE TABLE IF NOT EXISTS runs (
    ...
    commit_signing_policy   TEXT,
    commit_signing_effective INTEGER,
    ...
    launch_unsigned      INTEGER NOT NULL DEFAULT 0,
    pr_base_branch       TEXT,
    created_at           INTEGER NOT NULL,
...
var migrationStatements = []string{
	...
	`ALTER TABLE runs ADD COLUMN commit_signing_policy TEXT`,
	`ALTER TABLE runs ADD COLUMN commit_signing_effective INTEGER`,
	...
	// Explicit run-scoped unsigned admission is part of nonce proof identity.
	// It is separate from the effective signing boolean: a user's ordinary
	// default may also be false without having selected --unsigned.
	`ALTER TABLE runs ADD COLUMN launch_unsigned INTEGER NOT NULL DEFAULT 0`,
