...
func registerHandlers(srv *ipc.Server, mgr *RunManager, d *db.DB, shutdown func()) {
	...
	srv.Handle(ipc.MethodClaimLaunchReceipt, func(_ context.Context, params json.RawMessage) (interface{}, error) {
		...
		run, claimed, err := d.ClaimLaunchReceipt(p.RepoID, p.Branch, p.LaunchNonce, p.SubmittedHeadSHA, p.ValidationGeneration, p.IntentDigest, prBaseBranch, p.Unsigned)
		if err != nil {
		...
		if run.LaunchUnsigned != p.Unsigned {
			return nil, fmt.Errorf("conflicting launch_nonce %q is already bound to a different signing choice", p.LaunchNonce)
		}
	...
	srv.Handle(ipc.MethodRerun, func(ctx context.Context, params json.RawMessage) (interface{}, error) {
		...
		runID, err := mgr.HandleRerun(ctx, p.RepoID, p.Branch, p.PreviousRunID, p.SkipSteps, p.Intent, p.PRBaseBranch, p.CallerHeadSHA, p.Unsigned)
		if err != nil {
			return nil, err
		}
