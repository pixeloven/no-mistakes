...
func newAxiRunCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Triggers a pipeline run for the current branch and drives it. Without\n" +
			...
			"Commit signing follows the registered checkout by default. --unsigned explicitly\n" +
			"disables signing for this run through command-local Git options; it never writes,\n" +
			"clears, or requires persistent Git signing configuration.\n\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return trackAxiSurface("axi-run", "/axi/run", telemetry.Fields{
				...
				return runAxiRunWithLaunchProof(cmd, autoYes, skipSteps, intent, baseBranch, unsigned, launchNonce, validationGeneration, wait)
			})
		},
	}
...
func runAxiRunWithLaunchProof(cmd *cobra.Command, autoYes bool, skipSteps []types.StepName, intent, baseBranch string, unsigned bool, launchNonce, validationGeneration string, wait time.Duration) error {
	if err := validateAxiWait(wait); err != nil {
	...
	if launchNonce != "" {
		...
		receipt, err := claimLaunchReceipt(env.client, env.repo.ID, branch, launchNonce, headSHA, validationGeneration, digestLaunchIntent(intent), baseBranch, unsigned)
		if err != nil {
		...
		if active != nil {
			...
			if unsigned && !active.Unsigned {
				return emitError(cmd, 2, fmt.Sprintf("active run %s is already in progress without --unsigned", active.ID),
					"Omit --unsigned to reattach, or abort the active run before starting a new one")
			}
			runID = active.ID
		}
	}
	if runID == "" {
		...
		var err error
		if launchNonce != "" {
			launchReceipt, err = triggerProofRun(ctx, env, branch, headSHA, skipSteps, intent, baseBranch, unsigned, launchNonce, validationGeneration)
			if err == nil {
				runID = launchReceipt.RunID
			}
		} else {
			runID, err = triggerRun(ctx, env, branch, headSHA, skipSteps, intent, baseBranch, unsigned)
		}
		if err != nil {
...
func triggerRun(ctx context.Context, env *axiEnv, branch, headSHA string, skipSteps []types.StepName, intent, baseBranch string, unsignedValues ...bool) (string, error) {
	unsigned := len(unsignedValues) > 0 && unsignedValues[0]
	...
	if opt := formatUnsignedPushOption(unsigned); opt != "" {
		pushOptions = append(pushOptions, opt)
	}
...
func claimLaunchReceipt(client *ipc.Client, repoID, branch, launchNonce, submittedHeadSHA, validationGeneration, intentDigest, baseBranch string, unsigned ...bool) (*ipc.LaunchReceipt, error) {
	...
	if err := client.Call(ipc.MethodClaimLaunchReceipt, &ipc.ClaimLaunchReceiptParams{
		...
		Unsigned: len(unsigned) > 0 && unsigned[0],
	}, &result); err != nil {
		return nil, err
	}
...
func triggerProofRun(ctx context.Context, env *axiEnv, branch, headSHA string, skipSteps []types.StepName, intent, baseBranch string, unsigned bool, launchNonce, validationGeneration string) (*ipc.LaunchReceipt, error) {
	pushOptions := formatSkipPushOptions(skipSteps)
	...
	if receipt, err := waitForLaunchReceipt(ctx, env.client, env.repo.ID, branch, launchNonce, headSHA, validationGeneration, intent, baseBranch, triggerWaitTimeout, unsigned); err != nil {
		return nil, err
	} else if receipt != nil {
		return receipt, nil
	}
	...
	if err := env.client.Call(ipc.MethodStartFreshRun, &ipc.StartFreshRunParams{
		...
		Intent: intent, LaunchNonce: launchNonce, ValidationGeneration: validationGeneration, PRBaseBranch: baseBranch, Unsigned: unsigned,
	}, &result); err != nil {
...
func waitForLaunchReceipt(ctx context.Context, client *ipc.Client, repoID, branch, launchNonce, submittedHeadSHA, validationGeneration, intent, baseBranch string, timeout time.Duration, unsigned ...bool) (*ipc.LaunchReceipt, error) {
	deadline := time.NewTimer(timeout)
	defer deadline.Stop()
	poll := time.NewTicker(150 * time.Millisecond)
	defer poll.Stop()
	for {
		receipt, err := claimLaunchReceipt(client, repoID, branch, launchNonce, submittedHeadSHA, validationGeneration, digestLaunchIntent(intent), baseBranch, unsigned...)
		if err != nil {
			return nil, err
		}
		if receipt != nil {
...
func emitLaunchReceipt(cmd *cobra.Command, receipt ipc.LaunchReceipt) {
	emitDoc(cmd, toon.Field{Key: "launch_receipt", Value: toon.NewObject(
		...
		toon.Field{Key: "unsigned", Value: receipt.Unsigned},
	)})
}
