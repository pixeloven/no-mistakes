...
func TestAxiSyncRecoverKeepLocalRequiresAdvertisedAction(t *testing.T) {
	f := newCLIRecoverFixture(t)
	if err := os.WriteFile(filepath.Join(f.local, "rescope.txt"), []byte("rescope\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	var ee *exitError
	if err == nil || !asExitError(err, &ee) || ee.code != 1 {
		t.Fatalf("unadvertised keep-local should exit 1, got %#v\n%s", err, out)
	}
	for _, want := range []string{"safety: blocked_recover_manual_reconciliation", "code: inspect_and_reconcile_manually"} {
		if !strings.Contains(out, want) {
			t.Errorf("unadvertised refusal missing %q:\n%s", want, out)
		}
	}
	if after := cliRecoveryGitSnapshot(t, f); after != before {
		t.Fatal("unadvertised keep-local changed a branch, ref, or worktree")
	}
}
...
func TestAxiArchiveBackedRecoveryKeepsExactRequiredHeadAndBothHistories(t *testing.T) {
	...
	// The run-owned recovery ref independently proves the exact later head, so
	// recovery is already safe without trusting the unbound archive ref. Once
	// the operator binds that archive below, the immutable binding becomes the
	// authoritative boundary and must take priority over this run-owned anchor.
	...
	for _, want := range []string{
		...
		"recovery_ref: refs/no-mistakes/recover/" + f.runID,
		"archive_ref: \"\"",
		"keep_local: true",
		"proof: verified",
		"code: recover_custody",
	...
	if strings.Contains(status, "source: bound_archive") || strings.Contains(status, "archive_ref: "+f.archiveRef) {
		t.Fatalf("unbound archive was trusted instead of the exact run-owned anchor:\n%s", status)
	}
