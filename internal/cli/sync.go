...
func runAxiSync(cmd *cobra.Command, check, recover, keepLocal bool, bindArchiveRef string) error {
	...
	case check:
		state = service.Refresh(cmd.Context())
	case recover:
		// AXI recovery is command-by-state: do not let a caller guess the
		// keep-local mutation when the same fresh structured inspection does not
		// advertise it. Human `sync` retains its explicit confirmation flow.
		plan := service.InspectCached(cmd.Context())
		if keepLocal && plan.State != branchsync.StateUserOwned && plan.State != branchsync.StateCustodyReturned && !axiKeepLocalRecoveryAdvertised(plan) {
			state = plan
		} else {
...
func axiKeepLocalRecoveryAdvertised(state branchsync.State) bool {
	return state.NextAction != nil && state.NextAction.Code == "recover_custody" &&
...
func branchSyncField(state branchsync.State) toON.Field {
	...
	if state.Recovery != nil {
		fields = append(fields, toON.Field{Key: "recovery", Value: toON.NewObject(
			...
			toON.Field{Key: "recovery_ref", Value: state.Recovery.RecoveryRef},
