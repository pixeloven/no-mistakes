...
func (s *Service) Recover(ctx context.Context, keepLocal bool) State {
	...
	if objectExists(ctx, wd, preserved) && (local == preserved || isAncestor(ctx, wd, preserved, local)) {
		...
		gatePlan, refusal := s.planRecoveryGate(ctx, run, state, local)
		if refusal != nil {
			return *refusal
		}
		return s.finishRecover(ctx, run, state, false, gatePlan)
	}
	...
	if gateAnchorExists {
		gateAnchored, exists, err := git.ExactCommitRefTarget(ctx, gateDir, gateAnchor)
		if err != nil || !exists {
	...
	case isAncestor(ctx, wd, local, preserved):
		...
		gatePlan, refusal := s.planRecoveryGate(ctx, run, state, preserved)
		if refusal != nil {
			return *refusal
		}
		return s.recoverFastForward(ctx, run, state, preserved, gatePlan)
	default:
		if keepLocal {
			return s.recoverKeepLocalAtCurrentHead(ctx, run, state, []string{run.ID}, []string{run.HeadSHA})
		}
		if preservedContainsLocalWork(ctx, wd, local, preserved) {
			...
			return s.recoverAdoptPreserved(ctx, run, state, preserved, gatePlan)
		}
		state.Relation = RelationDiverged
...
func (s *Service) recoverKeepLocalFromArchive(ctx context.Context, run *db.Run, state State, gateHead string, archive *db.RecoveryArchive) State {
	...
	result := s.finishRecover(ctx, run, state, false, recoveryGatePlan{})
...
// recoveryGatePlan binds the gate branch state observed before an ordinary
// recovery moves the invoking worktree. Applying it later uses observed as an
// atomic compare-and-swap precondition, so a concurrent gate update wins and
// custody remains unstamped.
type recoveryGatePlan struct {
	available bool
	update    bool
	branchRef string
	observed  string
...
// planRecoveryGate proves that replacing an available gate branch with the
// recovered local head cannot discard work. A shutdown commonly leaves the
// gate at the submitted head while the detached pipeline head is its
// content-preserving rebase. An independently advanced, conflicting,
// symbolic, or non-commit gate branch refuses.
func (s *Service) planRecoveryGate(ctx context.Context, run *db.Run, state State, target string) (recoveryGatePlan, *State) {
	plan := recoveryGatePlan{target: target}
	gateDir := strings.TrimSpace(s.GateDir)
	if gateDir == "" {
		return plan, nil
	}
	...
	plan.available = true
	plan.branchRef = "refs/heads/" + state.Local.Branch
	gateHead, exists, err := git.ExactCommitRefTarget(ctx, gateDir, plan.branchRef)
	if err != nil {
		blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_gate_evidence", "the local gate branch is symbolic, non-commit, or unreadable; custody was not returned and neither branch moved")
		return recoveryGatePlan{}, &blocked
	}
	if exists && gateHead == target {
		return plan, nil
	}
	if exists && !gateRecoveryContainsLocalWork(ctx, s.workDir(), gateDir, gateHead, target) {
		blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_gate_diverged", "the local gate branch has work that the recovered head is not proven to contain; inspect the gate branch and run-owned recovery refs before returning custody; neither branch moved")
		return recoveryGatePlan{}, &blocked
	}

	plan.update = true
	plan.observed = gateHead
	if exists {
		backupRef := custody.RecoveryGateRef(run.ID)
		backup, backupExists, backupErr := git.ExactCommitRefTarget(ctx, gateDir, backupRef)
		if backupErr != nil || backupExists && backup != gateHead {
			blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_gate_evidence", "the run's preserved gate-head evidence conflicts with the gate branch that recovery would replace; custody was not returned and neither branch moved")
			return recoveryGatePlan{}, &blocked
		}
	}
	return plan, nil
...
// applyRecoveryGatePlan makes the gate agree with the recovered local branch
// without a push, so the receive hook cannot start a run. Every replaced gate
// head is create-only anchored first. The target is imported through an exact
// temporary ref that FetchRemoteRef always removes, then the branch update is
// compare-and-swapped against the plan's observation and verified.
func (s *Service) applyRecoveryGatePlan(ctx context.Context, run *db.Run, state State, changed bool, plan recoveryGatePlan) (State, bool) {
	if !plan.available || !plan.update {
		return State{}, true
	}

	branch, branchErr := git.CurrentBranch(ctx, s.workDir())
	head, headErr := git.HeadSHA(ctx, s.workDir())
	if branchErr != nil || branch != state.Local.Branch || headErr != nil || head != plan.target {
		blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_assumptions_changed", "the recovered local branch changed before the gate could be reconciled; custody was not recorded")
	...
	if plan.observed != "" {
		backupRef := custody.RecoveryGateRef(run.ID)
		if err := custody.PreserveRecoveryAnchor(ctx, s.GateDir, backupRef, plan.observed); err != nil {
			blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_gate_evidence", "the previous gate head could not be preserved before reconciliation; custody was not recorded")
			blocked.Changed = changed
			return blocked, false
		}
	}
	...
	source, err := filepath.Abs(s.workDir())
	if err != nil {
		blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_assumptions_changed", "the invoking worktree path could not be resolved before gate reconciliation; custody was not recorded")
		blocked.Changed = changed
		return blocked, false
	}
	if err := git.FetchRemoteRef(ctx, s.GateDir, source, plan.branchRef, plan.target); err != nil {
		blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_gate_update", "the recovered local head could not be imported exactly into the gate; custody was not recorded")
		blocked.Changed = changed
		return blocked, false
	}
	if err := git.ValidateExactCommit(ctx, s.GateDir, plan.target); err != nil {
		blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_gate_update", "the recovered local head could not be verified in the gate; custody was not recorded")
		blocked.Changed = changed
		return blocked, false
	}
	...
	old := plan.observed
	if old == "" {
		old = strings.Repeat("0", len(plan.target))
	}
	if _, err := git.Run(ctx, s.GateDir, "update-ref", "--no-deref", plan.branchRef, plan.target, old); err != nil {
		blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_gate_race", "the gate branch changed while custody was being returned; all preserved heads remain anchored and custody was not recorded")
		blocked.Changed = changed
		return blocked, false
	}
	settled, exists, err := git.ExactCommitRefTarget(ctx, s.GateDir, plan.branchRef)
	if err != nil || !exists || settled != plan.target {
		blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_gate_update", "the gate branch could not be verified at the recovered head; all preserved heads remain anchored and custody was not recorded")
		blocked.Changed = changed
		return blocked, false
	}
	return State{}, true
...
func (s *Service) recoverFastForward(ctx context.Context, run *db.Run, state State, preserved string, gatePlan recoveryGatePlan) State {
	...
	return s.finishRecover(ctx, run, state, true, gatePlan)
}
...
// gateRecoveryContainsLocalWork evaluates the same content-preservation proof
// with the gate object database available to the invoking worktree. This is
// needed before the recovered local target has been imported into the bare
// gate. Every unreadable or incomplete object relationship fails closed.
func gateRecoveryContainsLocalWork(ctx context.Context, localDir, gateDir, gateHead, recoveredHead string) bool {
	gateObjects, err := filepath.Abs(filepath.Join(gateDir, "objects"))
	if err != nil {
		return false
	}
	env := []string{"GIT_ALTERNATE_OBJECT_DIRECTORIES=" + gateObjects}
	gateType, gateErr := git.RunWithEnv(ctx, localDir, env, "cat-file", "-t", gateHead)
	recoveredType, recoveredErr := git.RunWithEnv(ctx, localDir, env, "cat-file", "-t", recoveredHead)
	if gateErr != nil || recoveredErr != nil || gateType != "commit" || recoveredType != "commit" {
		return false
	}
	if gateHead == recoveredHead {
		return true
	}
	if _, err := git.RunWithEnv(ctx, localDir, env, "merge-base", "--is-ancestor", gateHead, recoveredHead); err == nil {
		return true
	}
	base, err := git.RunWithEnv(ctx, localDir, env, "merge-base", gateHead, recoveredHead)
	if err != nil || base == "" {
		return false
	}
	mergedTree, err := git.RunWithEnv(ctx, localDir, env, "merge-tree", "--write-tree", "--merge-base", base, recoveredHead, gateHead)
	if err != nil {
		return false
	}
	recoveredTree, err := git.RunWithEnv(ctx, localDir, env, "rev-parse", recoveredHead+"^{tree}")
	return err == nil && mergedTree == recoveredTree
}
...
func (s *Service) recoverAdoptPreserved(ctx context.Context, run *db.Run, state State, preserved string, gatePlan recoveryGatePlan) State {
	if s.beforeRecoverWorktreeMove != nil {
		s.beforeRecoverWorktreeMove()
	}
	wd := s.workDir()
...
// finishRecover settles the available internal gate branch, stamps custody
// returned, and reports the fresh post-recovery truth. changed reports whether
// this call moved the worktree HEAD.
func (s *Service) finishRecover(ctx context.Context, run *db.Run, state State, changed bool, gatePlan recoveryGatePlan) State {
	if blocked, ok := s.applyRecoveryGatePlan(ctx, run, state, changed, gatePlan); !ok {
		return blocked
	}
...
func (s *Service) supersededUnpublishedRun(ctx context.Context, older, newer *db.Run, branch string) bool {
	if older == nil || newer == nil || !terminalRunStatus(older.Status) || !unpublishedPipelineHead(older) ||
		!samePushTargetBinding(older, newer) || strings.TrimSpace(s.GateDir) == "" ||
		!validFullObjectIDForRepo(ctx, s.GateDir, older.HeadSHA) || newer.LastPushedSHA == nil ||
		!validFullObjectIDForRepo(ctx, s.GateDir, *newer.LastPushedSHA) {
		return false
	}
...
func (s *Service) missingHeadKeepLocalRuns(ctx context.Context, state *State, run *db.Run) ([]string, []string, bool, bool) {
	...
	for _, candidate := range runs {
		...
		eligible := candidate.TerminalHeadVerifiedAt != nil && validFullObjectIDForRepo(ctx, s.workDir(), candidate.HeadSHA)
...
func (s *Service) recoverySourceAvailable(ctx context.Context, state *State, run *db.Run) recoverySourceProof {
	...
	if !validFullObjectIDForRepo(ctx, s.workDir(), run.HeadSHA) {
		return unavailableRecoverySource("blocked_recover_preserved_head_invalid", fmt.Sprintf("the recorded pipeline head %q is not a valid full object ID; no files or refs were changed", run.HeadSHA))
	}
	...
	if len(archiveRecords) > 0 {
		...
		// A bound archive is the narrower proof: it authorizes only returning
		// custody at its recorded required head. Do not let a coincident gate
		// anchor broaden that transaction or bypass its recovery-boundary CAS.
		return archiveProof
	}
	...
	if !ordinaryAvailable && gateAvailable && state.Local.Clean && gatePreservedHeadSource(ctx, gateDir, run) == recoverySourceStateAvailable {
		ordinaryAvailable = gateRecoveryContainsLocalWork(ctx, s.workDir(), gateDir, local, preserved)
	}
...
// exactAnchoredDivergence proves the narrow keep-local recovery offer for a
// clean branch whose local and terminal pipeline heads genuinely diverged.
// The terminal head must be verified and named by its exact, non-symbolic,
// run-owned gate ref. The gate branch and any displaced-head anchor are also
// preflighted so the advertised command is one Recover can execute without
// discarding a ref.
func (s *Service) exactAnchoredDivergence(ctx context.Context, state *State, run *db.Run) bool {
	if state == nil || run == nil || s.Repo == nil || !state.Local.Clean ||
		run.TerminalHeadVerifiedAt == nil || run.CustodyReturnedAt != nil ||
		state.Local.Branch != run.Branch || state.Local.Head == "" || state.Local.Head == run.HeadSHA {
		return false
	}
	if git.ValidateExactCommit(ctx, s.workDir(), state.Local.Head) != nil {
		return false
	}
	gateDir := strings.TrimSpace(s.GateDir)
	if gateDir == "" {
	...
	anchorRef := custody.RecoveryRef(run.ID)
	anchored, exists, err := git.ExactCommitRefTarget(ctx, gateDir, anchorRef)
	if err != nil || !exists || anchored != run.HeadSHA {
		return false
	}

	gateBranchRef := "refs/heads/" + state.Local.Branch
	gateHead, gateExists, err := git.ExactCommitRefTarget(ctx, gateDir, gateBranchRef)
	if err != nil {
		return false
	}
	if gateExists && gateHead != state.Local.Head && gateHead != run.HeadSHA {
		compatible, err := recoveryGateAnchorCompatible(ctx, gateDir, run.ID, gateHead)
		if err != nil || !compatible {
			return false
		}
	...
	env := []string{"GIT_ALTERNATE_OBJECT_DIRECTORIES=" + gateObjects}
	if localType, err := git.RunWithEnv(ctx, s.workDir(), env, "cat-file", "-t", state.Local.Head); err != nil || localType != "commit" {
		return false
	}
	if preservedType, err := git.RunWithEnv(ctx, s.workDir(), env, "cat-file", "-t", run.HeadSHA); err != nil || preservedType != "commit" {
		return false
	}
	if _, err := git.RunWithEnv(ctx, s.workDir(), env, "merge-base", "--is-ancestor", state.Local.Head, run.HeadSHA); err == nil {
		return false
	}
	if _, err := git.RunWithEnv(ctx, s.workDir(), env, "merge-base", "--is-ancestor", run.HeadSHA, state.Local.Head); err == nil {
		return false
	}
	base, err := git.RunWithEnv(ctx, s.workDir(), env, "merge-base", state.Local.Head, run.HeadSHA)
	return err == nil && base != ""
}
...
const (
	recoverySourceStateAvailable recoverySourceState = iota
	recoverySourceStateMissing
	recoverySourceStateInvalid
)
...
func gatePreservedHeadSource(ctx context.Context, gateDir string, run *db.Run) recoverySourceState {
	if run == nil || strings.TrimSpace(run.HeadSHA) == "" || strings.TrimSpace(gateDir) == "" {
		return recoverySourceStateInvalid
	}
	...
	if target, exists, targetErr := git.ExactRefTarget(ctx, gateDir, anchor); targetErr == nil && exists {
		if compatible && target == run.HeadSHA {
			return recoverySourceStateAvailable
		}
		return recoverySourceStateInvalid
	}
	if !compatible {
	...
	if err != nil {
		return recoverySourceStateMissing
	}
...
func localRecoveryEligible(ctx context.Context, wd string, state *State, run *db.Run) bool {
	return exactCommitExists(ctx, wd, run.HeadSHA) &&
...
func exactCommitRefCompatible(ctx context.Context, repoDir, ref, expected string) (bool, error) {
	...
	target, exists, err := git.ExactCommitRefTarget(ctx, repoDir, ref)
	if err != nil {
		return false, err
	}
	if !exists {
...
func (s *Service) classifyCustodyReturned(ctx context.Context, state *State) {
	...
	if gateDir != "" {
		if _, err := os.Stat(gateDir); err == nil {
			...
			gateHead, exists, err := git.ExactCommitRefTarget(ctx, gateDir, gateRef)
			if err != nil {
				state.Safety = "blocked_custody_returned_gate_invalid"
				state.Error = "custody is recorded as returned, but the internal gate branch is symbolic, non-commit, or unreadable; preserve and reconcile that evidence before starting another run"
				state.NextAction = &NextAction{Code: "inspect_and_reconcile_manually", Command: "no-mistakes axi status"}
				return
			}
			if exists && gateHead != state.Local.Head {
				state.Safety = "blocked_custody_returned_gate_stale"
				state.Error = fmt.Sprintf("custody is recorded as returned, but internal gate branch %s remains at %s instead of local head %s; leave both refs unchanged and launch an unused successor branch at the exact local head", state.Local.Branch, gateHead, state.Local.Head)
				state.NextAction = &NextAction{Code: "create_successor_branch", Command: "git switch -c <unused-successor-branch> " + state.Local.Head}
				return
			}
		}
	}
